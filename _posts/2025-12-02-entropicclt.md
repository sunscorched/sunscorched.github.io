...
In our example above, we have Bernoulli random variables $B_i$ with mean $p=0.9$. If we let $X_i=B_i -0.9$, then $E[X_i] = 0$, $E[X^2_i] = E[(B_i-0.9)^2] = E[B_i^2]-(0.9)^2$. Now, $B^2_i = B_i$ since $0^2=0,1^2=1$, then we have $E[X^2_i] = 0.9 - 0.81 =pq= 0.09 = \sigma^2$. So $\sigma = 0.3$. Lastly, $|B_i-p|^3$ can take on value $|0-p|^3 = p^3$ with probability $q$ or $|1-p|^3 = q^3$ with probability $p$. So $\rho = pq(p^2+q^2) = 0.0738$. Thus, for $n=250$, the error is bounded above by constant $C$ times the rest which equals approximately $0.17287$. So the final bound is about $0.0821$. In terms of $n$, the bound is about $1.298 n^{-1/2}$.
**Corollary:** Suppose that $H[f_n]$ is finite for some $n$. Then $\lim \int|f_n(x)-\phi(x)|\,dx = 0$; i.e. the $f_n \to \phi$ in $L^1$ and hence in probability.
