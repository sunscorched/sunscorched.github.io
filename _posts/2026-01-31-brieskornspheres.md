...
**Theorem (Levine):** For $4m+1$, $(m \geq 1)$, let $\Sigma_a$ be the homotopy sphere where $a=(2,2,...,2,3)$. This has Kervaire invariant 1 and if the subgroup of homotopy spheres bounding parallelizable manifolds is $bP_{4m+2}=\Z/2$, then $\Sigma_a$ generates $bP_{4m+2}$.
