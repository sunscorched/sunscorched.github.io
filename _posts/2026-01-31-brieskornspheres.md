...
Recall that a homotopy $n$-sphere is a smooth $n$-manifold homotopy equivalent to $S^n$. For $n \neq 4$, these form finite abelian groups $\Theta_n$ under connected sum and contain **cyclic** subgroups $bP_{n+1}$ of the homotopy $n$-spheres that bound parallelizable manifolds. By surgery, we can ensure that the bounding manifolds have vanishing $i$th homotopy groups for $i<n/2$. When $n$ is even, we can in fact show that any bounding manifold is contractible. But by the h-cobordism theorem, there are no exotic disks; i.e. every contractible manifold with boundary homeomorphic to a sphere must in fact be diffeomorphic to the standard disk. Thus, when $n$ is even, only the standard sphere bounds a parallelizable manifold and $bP_{2n+1}=0$. 

In dimensions $n=4m$, the intersection form is symmetric and the signature is the main invariant of interest. In dimensions $n=4m+2$, the intersection form is skew-symmetric and the Kervaire invariant (which only takes values 0 or 1) is the one of interest. These two invariants act as obstructions to surgering a bounding manifold into a ball of the appropriate dimension. In particular, $bP_{4m+2}$ is always either trivial or $\mathbb{Z}/2$. Now, [Kervaire-Milnor](https://www.uni-math.gwdg.de/schick/publ/Groups%20of%20homotopy%20spheres%20I.pdf) computed many of these groups and Brieskorn's paper implies the following:
