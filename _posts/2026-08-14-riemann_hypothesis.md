...
In 1914, Hardy showed that there are infinitely zeros on the critical line. Later, Selberg in 1942 showed something about the asymptotic density of nontrivial zeros. That is, let $T>0$ be a height at which we cut off the critical strip. So we have a compact set with finitely many zeros. If there were infinitely, because the function is analytic and the zeros accumulate, you'd have the constant function 0. Anyways, we can count the number of zeros on the critical line in this compact set, call that count $N_0(T)$ and also count the total number of zeros in the compact set $N(T)$. Selberg showed that for all sufficiently large $T$, $N_0(T)\geq \kappa N(T)$ where $\kappa >0$ is some constant. This means that $\liminf_{T \to \infty} \dfrac{N_0(T)}{N(T)} \geq \kappa$. It is not known if the limit of ratios actually exists; there could be a lot of variation. But we at least have a lower bound on how large the variation can be.

If we can show $\kappa = 1$, the Riemann Hypothesis is still not proved but it's good progress. Mathematicians have worked to improve lower bounds on $\kappa$. Prior to Anthropic's announcement, it was known that $\kappa\geq 5/12$. Anthropic is claiming that $\kappa \geq 8/12 = 2/3$. I can give no comments on the proof as I know very little number theory. But it is very interesting to see Claude (the Anthropic model) write a paper like a human would. For example, it wrote an acknowledgement section, thanking one of Anthropic's employees for the support and encouragement.

**Update (8 September, 2026):** A few days ago, mathematician Youness Lamzouri posted a [preprint](https://arxiv.org/abs/2609.02882) of a paper with a simpler proof that shows a stronger result than Claude. He achieved the lower bound of $269/400=0.6725$.
