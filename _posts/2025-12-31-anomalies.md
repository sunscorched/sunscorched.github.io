...
**Remark:** Over any field, a linear transformation $A:W \to W$ induces a map $\mathbb{P}(A): \mathbb{P}(W)\to\mathbb{P}(W)$ since linear maps send lines to lines. For example, let $R_\theta:\mathbb{R}^2 \to \mathbb{R}^2$ rotate the plane by an angle $\theta$. Then the induced map on $\mathbb{RP}^1$. When we identify $\mathbb{RP}^1$ with $S^1$, the map is $e^{2 i \theta}$. For example, if the linear map rotates by $\pi$, then the map on $\mathbb{RP}^1$ is the identity.
