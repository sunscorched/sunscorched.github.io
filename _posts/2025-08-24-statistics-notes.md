...
But the sum of squared normal standard deviations, by definition, is just $\chi^2_{n-p}$. Since $E[Z^2_i] = \text{Var}(Z_i) + (E[Z_i])^2 = 1^2 + 0$, then $E[\chi^2_{n-p}] = n-p$. So $E[\text{RSS}/\sigma^2] = E[\chi^2_{n-p}]=n-p$ and hence, $\sigma^2 = \frac{1}{n-p}E[\text{RSS}]$. Hence, $s^2 = \frac{1}{n-p}\text{RSS}$ is an unbiased estimator of the variance.
