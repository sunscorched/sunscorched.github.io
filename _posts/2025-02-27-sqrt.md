...
The first two terms cancel and so we have $e\_{k+1} \approx \dfrac{f''(r)}{2f'(r)} e^2\_k$. In the case of $f(x) = x^2-a$, algebra shows that $e\_{k+1} = \dfrac{1}{2(r+e_k)}e^2\_k$ and we can assume that $r\geq 1$ since we were only looking for the integer square root.
