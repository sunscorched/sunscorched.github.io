...
There are many Lagrange points in our solar system (and indeed, whenever we have two large, gravitating bodies). For example, we have 5 Lagrange points associated to the system where the earth and sun are primaries. Since they are stable points, we've put objects at those points. For example, at $L_1$, the Deep Space Climate Observatory (DSCOVR) is studying solar wind. At $L_2$, the James Webb Space Telescope is taking pictures of faraway objects to understand the cosmological past. Both of these points sit about 930,000 miles from earth which is quite far but still only about 1% the distance to the sun. Jupiter and Saturn also have such Lagrange points as do the earth and moon. 

Why not put them at $L_4,L_5$, which are more stable and satellites and asteroids can hang out at these points more easily? One reason is that $L_2$ is yielded from the sun by Earth and the James Webb needs darkness in order to capture distant light. It periodically makes small adjustments with thrusters. The $L_4,L_5$ points experience too much thermal radiation.

That being said, the $L_4$ and $L_5$ points are certainly of interest and bodies at those two points are sometimes called "Trojans," because they are chasing each other like Achilles chased Hector around Troy to avenge the death of Patroclus. Hence, the $L_4$ bodies are called the "Hector group" and the $L_5$ bodies are called the "Achilles group". These bodies were first found near Jupiter. If the mass of the two celestial bodies start to get closer so that the mass of the larger one is less than some constant $C \approx 25$ times the mass of the other, then even the $L_4,L_5$ Lagrange points become unstable. 

**Claim:** $C = (25+3\sqrt{69})/2 \approx 24.96$.

**Proof:** Let the $M_1 \geq M_2$ be the two masses of the large bodies and define the mass parameter $\mu = \dfrac{M_2}{M_1+M_2} \leq 0.5$. If we linearize the equations of motion at $L_4$ and $L_5$, the resulting differential equations give a characteristic polynomial $x^4 +x^2 + \dfrac{27}{4}\mu(1-\mu)$ whose roots are the eigenvalues. There were 6 degrees of freedom but remember, we set $q_3=p_3=0$. For the Lagrange points to be stable, we do not want any exponential terms and so we'd want the roots to be purely imaginary (which gives periodic behavior). Note this quartic becomes a quadratic with substitution $y=x^2$; the roots of that quadratic are $-\dfrac{1}{2} \pm \dfrac{\sqrt{1-27\mu(1-\mu)}}{2}$.

To have purely imaginary roots, we'd want the discriminant to be non-negative which is equivalent to $27\mu^2-27\mu+1 \geq 0$. The boundary of this occurs at $\mu_b = \dfrac{9-\sqrt{69}}{18}$. We take the smaller value because the other one is larger than $0.5$. Next, note that $\dfrac{M_1}{M_2}=\dfrac{1-\mu}{\mu}$. Plugging in $\mu_b$, we find that the ratio is $\dfrac{M_1}{M_2}=\dfrac{25+3\sqrt{69}}{2}$. $\square$

Moving forward, let's just use the earth and moon as examples. Looking at the Hamiltonian $H$ again, note that there is noncompactness for the 5-dim level sets of $H$. For instance, when $q = E$ or $q=M$ corresponds to the sattelite crashing into the earth or moon and the potential energy diverges. One way to deal with this is to compactify our position space and work with a new Hamiltonian $Q:T^\*S^3 \to \mathbb{R}$ which agrees with the original Hamiltonian away from the newly added points. Roughly speaking, the way to arrive at $T^\*S^3$ is to do a trick due to Moser of interchanging $p$ and $q$. Then where $p=\infty$ which represents collision, we do a real oriented blow-up to add in all the directions in which the collision can occur; i.e. add in a copy of $S^2$. Moreover, the energy hypersurface (level set of $Q$) can be described topologically. For $c \in \mathbb{R}$, consider $\Sigma_c = H^{-1}(c)$. For details, see Moreno's notes.
