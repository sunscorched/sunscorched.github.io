...
The Hamiltonian we would want to study is then the kinetic energy plus the two gravity potentials but it is time-dependent. To remedy this, we may choose rotating coordinates which puts the primaries at rest but the cost is to introduce centrifugal and Coriolis effects. The way to do this is introduce a term which adds a rotation of the plane to the Hamiltonian. The term $p_1q_2-p_2q_1$ does the trick. Hence, the Hamiltonian is now autonomous:

$$H: \R^3 \setminus \{E, M\} \times \R^3 \to \R$$ 
$$H(q,p) = \frac{1}{2}\|p\|^2 − \frac{\mu}{\|q -M\|} -\frac{\mu}{\|q -E\|} + p_1q_2 - p_2q_1$$

We can restrict our attention to when $q_3=p_3=0$ in order to study the planar problem; the flow preserves this subset. We also have two antisymplectic involutions:
$$\rho(q_1,q_2,q_3,p_1,p_2,p_3)=(q_1,-q_2,q_3,-p_1,p_2,-p_3)$$
$$\bar{\rho}(q_1,q_2,q_3,p_1,p_2,p_3)=(q_1,-q_2,-q_3,-p_1,p_2,p_3)$$

The fixed points of these involutions are Lagrangians. Also, note that $\sigma = \rho \circ \bar{\rho}$ is a symplectic involution which adds minus signs to $q_3$ and $p_3$. So the fixed point locus of $\sigma$ is where the planar problem resides. This Hamiltonian has 5 fixed points which are called Lagrange points.

![label](/files/lagrange_points.png)

The points in the diagram are labeled so that their energy is in increasing order when the mass ratio $\mu < 1/2$; i.e. $H(L_1)<H(L_2)<H(L_3)<H(L_4)<H(L_5)$. When $\mu=1/2$, we have that $H(L_2)=H(L_3)$. $L_1,L_2,L_3$ are saddle points while $L_4,L_5$ are local maxima. In the plane, these two actually form equilateral triangles with $E$ and $M$ as the other vertices!

We note that there can be a lot of noncompactness for the 5-dim level sets of $H$. For instance, when $q = E$ or $q=M$ corresponds to the sattelite crashing into the earth or moon and the potential energy diverges. One way to deal with this is to compactify our position space and work with a new Hamiltonian $Q:T^*S^3 \to \R$ which is like the original Hamiltonian away from the newly added points.

The way to arrive at $T^*S^3$ is to do a trick of interchanging $p$ and $q$. Then where $p=\infty$ which represents collision, we do a real oriented blow-up to add in all the directions in which the collision can occur; i.e. add in a copy of $S^2$.

The energy hypersurface (level set of $Q$) can be described topologically. For $c \in \R$, consider $\Sigma_c = H^{-1}(c)$.

If $\pi: \R^3 \setminus \{E, M\} \times \R^3 \to \R^3 \setminus \{E, M\}$, $\pi(q, p) = q$ is projection onto the position coordinate, we define the **Hill's region** of energy $c$ as $K_c = \pi(\Sigma_c)$. This is the region in space where the satellite with energy $c$ is allowed to move. If $c < H(L_1)$ lies below the first critical energy value, then $K_c$ has three connected components: a bounded one around the Earth, another bounded one around the Moon, and an unbounded one. Namely, if the satellite starts near one of the primaries, and has low energy, then it stays near the primary also in the future. The unbounded region corresponds to asteroids which stay away from the primaries. Denote the first two components by $K^E_c$ and $K^M_c$ and $\Sigma^E_c = \pi^{-1}(K^E_c) \cap \Sigma_c$ and similarly for $\Sigma^M_c = \pi^{-1}(K^M_c) \cap \Sigma_c$.

When $c$ crosses the 1st critical value $H(L_1)$, then $\Sigma^E_c$ and $\Sigma^M_c$ get surgered together by connected sum and we get a new hypersurface $\Sigma^{E,M}_c$. We may compactify these in $T^*S^3$ to get $\overline{\Sigma}^E_c$ and $\overline{\Sigma}^M_c$. Let $S^*S^3 = S^2 \times S^3$ be the sphere bundle. Then $\overline{\Sigma}^E_c \cong S^*S^3 \#S^*S^3$.

In the planar problem, the situation is similar: we compactify $\R^2$ to $S^2$ and obtain copies of $S^*S^2 = \mathbb{RP}^3$ as energy hypersurfaces. There is a way to study these by using the double cover $S^3 \to \mathbb{RP}^3$. Of course, these energy hypersurfaces are contact manifolds where the contact form comes from the exact symplectic form on $T^*S^3$ or $T^*S^2$.

### The Planar Problem for CR3BP

G. W. Hill found, when studying the movement of the moon, that there is a direct orbit for the moon: it rotates in the same direction around the Earth as the Earth around the Sun. There is also the opposite situation is a retrograde motion.

Poincaré was very impressed with this study and he introduced a way to study the system using these two orbits.
...
The main point here is that we had a continuous dynamical system but we've reduced our study of it to looking at a discrete problem. To see if there is stable behavior, we can study the 1st return map, also called the **Poincaré return map** and see where a point goes under many iterates. Surfaces of section have a symplectic structure and this map is a Hamiltonian diffeomorphism on that surface.

Back to the direct and retrograde orbits of the moon. Using the double cover $S^3 \to \mathbb{RP}^3$, these two orbits lift to a pair of unknots in $S^3$ but they are linked to each other; i.e. we have the Hopf link which bounds an annulus $A$.

![label](/files/annulus.png)
This picture is from Alain Chenciner's [article](http://www.bourbaphy.fr/chenciner.pdf), p. 59.

Since the direction of orbit of the direct and retrograde orbits are opposite to each other, the Poincaré return map is an area preserving map of the annlus $A$ which **twists**; i.e. it rotates the boundary components in opposite directions. Enter now the famous theorem studied by Poincaré in many cases and proved by Birkhoff shortly after Poincaré's death:
...
For any kind of self-map $f:X \to X$, a point is periodic if there exists a positive integer $n$ such that $f^n(x)=x$; i.e. $n$ iterates of $f$ brings $x$ back to itself.
...
This means that there are infinitely many stable planar orbits for a satellite orbiting in the earth and moon system.

I will try to outline a proof of this theorem and also the corollary to just find one fixed point under an additional assumption and also assuming we're working with diffeomorphisms (the second of these is pretty mild). We will follow McDuff-Salamon's _Introduction to Symplectic Topology._

Before doing that, I want to make a few remarks.
1. In order to find the direct orbit away from the lunar problem, Birkhoff had in mind finding a disk-like surface of section whose boundary is precisely the retrograde orbit. The direct orbit would then be found via Brouwer's translation theorem: every area preserving homeomorphism of the open disk admits a fixed point. Removing the fixed point, we obtain an area preserving homeomorphism of the open annulus, which, via a theorem of Franks, admits either none or infinitely many periodic points. Combining these, one has: an area preserving homeomorphism of an open disk admits either one or infinitely many periodic points. Note that if the boundary is also an orbit, we obtain 2 or infinitely many periodic points. If furthermore we have the twisting, the Poincaré–Birkohff theorem then provides infinitely many orbits.
2. The Poincaré–Birkhoff theorem is really a beautiful result. It is easy to state, difficult to prove, none of the assumptions can be removed, and the conclusion is sharp. That is, we can find examples of diffeomorphism which are area and orientation preserving but do not twist and lack fixed points. We can also find diffeomorphisms satisfying orientation preserving and twisting but not area preserving which also lack fixed points. There are also diffeomorphisms which are area preserving and twisting but orientation reversing which lack fixed points. Lastly, we can form diffeomorphisms satisfying all the properties with exactly 2 fixed points.
3. We can also use modern techniques of Floer theory. Glue two copies of the annulus together to form a torus $T^2$ which now has a Hamiltonian diffeomorphism $\Psi:T^2 \to T^2$ acting on it, defined by just $\psi$ acting on each annulus. If $\Psi$ is nondegenerate, then by using Hamiltonian Floer theory, we know that the number of orbits is lower bounded by the rank of the homology of $T^2$ which is 4. Since $\Psi$ is built from two copies of $\psi$, there must be an even number of fixed points. Since $\psi$ has no fixed points on the boundary (it's supposed to twist them), the fixed points must be on the interior and there must be at least 2 for on $\psi:A\to A$. This technique doesn't help us understand degenerate Hamiltonians, however.

### Proof of Poincaré-Birkhoff under a monotone twisting condition

Let's use polar coordinates to describe $A = \R/\Z \times [a,b]$. The homeomorphism $\psi(x,y) = (f(x,y),g(x,y))$ depends on two functions where $f(x+1,y) = f(x,y)+1$ and $g(x+1,y)=g(x,y)$ because of periodicity. Since $\psi$ preserves the boundary components, $g(x,a)=a,g(x,b)=b$. Because it twists the boundary in opposite directions, then let's suppose that $f(x,a)<x$ and $f(x,b)>x$. So the inner boundary is twisted clockwise (decrease the angle) and the outer boundary is twisted counterclockwise.

Two points $(x,y)$ and $(x',y')$ are **geometrically distinct** if either $y \neq y'$ or $x-x' \notin \Z$. If we lift $\psi$ to the universal cover $\R \times[a,b]$, then its functions $f',g'$ will have the same properties as above. Also, the lift satisfies the twist condition and is orientation preserving if and only if it preserves the two boundaries of $\R\times [a,b]$.

The **monotone twist condition** is the following: if $y<y'$, then $f(x,y)< f(x,y')$. So if we foliate the annulus into circles of increasing radius, those of larger radii get twisted counterclockwise more than those of smaller radii. Note that $\psi$ doesn't necessarily send these circles to other circles.

Now, under this condition, there exists a continuous function $w:\R \to [a,b]$ such that $f(x,w(x))=x$ and moreover, since $f(x+1,y)=f(x,y)+1$, then $x+1=f(x+1,w(x+1)) = f(x,w(x+1))+1$. So $f(x,w(x+1))=x=f(x,w(x))$. All that to say, $w(x+1)=w(x)$.

Define the curve closed curve $\Gamma = \{(x,w(x)):x\in\R\}$. Its image $\psi(\Gamma)$ under $\psi$ must intersect $\Gamma$ in at least two points. To see this, suppose there is no intersection. The picture below shows this hypothetical situation; let the blue curve be $\Gamma$ and the red is $\psi(\Gamma)$. Then the region between the blue curve and the inner boundary is mapped to the region between the red curve and the inner boundary; the area has increased which violates the area preserving property of $\psi$. If they intersect at only one point (a sort of tangency point), the same area argument applies. Of course, it's also not important that $\psi(\Gamma)$ was enlarged; the same argument works if $\Gamma$ were on the outside and $\psi(\Gamma)$ on the inside.
...
**Remark:** This argument crucially relies on the monotone twisting condition; the argument fails without it.
