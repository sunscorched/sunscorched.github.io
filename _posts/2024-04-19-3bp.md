...
We note that there can be a lot of noncompactness for the 5-dim level sets of $H$. For instance, when $q = E$ or $q=M$ corresponds to the sattelite crashing into the earth or moon and the potential energy diverges. One way to deal with this is to compactify our position space and work with a new Hamiltonian $Q:T^*S^3 \to \mathbb{R}$ which is like the original Hamiltonian away from the newly added points.
...
The energy hypersurface (level set of $Q$) can be described topologically. For $c \in \mathbb{R}$, consider $\Sigma_c = H^{-1}(c)$.
