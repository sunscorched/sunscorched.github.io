...
This is a follow-up on Donaldson's work in gauge theory.

Since all K3 surfaces are diffeomorphic by Kodaira's work, we can use any K3 for our purposes. Recall that $(b^+_2,b^-_2) = (3,19)$ since the intersection form is $3H\oplus 2(-E_8)$.
...
$-p_1(P) = 4c_2(E) - c_1(E)^2=2(4c_2(\mathbb{CP}^2) - c_1(\mathbb{CP}^2)^2) = 2(4*3-9)=6$. The extra 2 comes from the fact that $\pi$ is a branched double cover. So $p_1(P)=-6$. Also, $\alpha = w_2(P) = \pi^* h$ where $h$ generates $H^2(\mathbb{CP}^2)$. Since $\alpha \cdot \alpha = 1$ and $\pi$ is a deg 2 map, then $\alpha^2 \equiv 2 \pmod{4}$.
